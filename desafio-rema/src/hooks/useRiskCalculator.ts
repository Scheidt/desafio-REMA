...
import { ERROR_MESSAGES, RESULT_MESSAGES, RISK_THRESHOLDS, DISPLAY_PRECISION } from '@/constants';
...
export function useRiskCalculator(): UseRiskCalculatorReturn {
  ...
  const [mapData, setMapData] = useState<Map<string, { RfDo: number | undefined }>>(new Map());
  ...
  const loadDataFromXlsx = useCallback(async () => {
    try {
      ...
      setGlobalError('Failed to load contaminant data. Please refresh the page.');
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const updateField = useCallback((field: keyof UserArguments, value: string | number) => {
  ...
  const handleCalculate = useCallback(() => {
    ...
    try {
      ...
      const resultMessage = risk >= RISK_THRESHOLDS.HIGH_RISK 
